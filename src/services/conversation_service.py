...
class ConversationService:
    ...
    async def get_conversation_detail(
        ...
        Example:
            ...
            detail = await service.get_conversation_detail(
                conv_id, user_id, message_limit=20
            )

            # Get conversation metadata only (no messages)
            detail = await service.get_conversation_detail(
                conv_id, user_id, include_messages=False
            )
            ```
        """
        logger.info(
    ...
    async def delete_conversation(
        ...
        Example:
            ...
            if success:
                ...
                print("Conversation not found or unauthorized")
            ```
        """
        logger.info(
    ...
    async def get_conversation_context(
        ...
        Example:
            ...
            context = await service.get_conversation_context(conv_id, user_id)

            # Get last 10 messages
            context = await service.get_conversation_context(
                conv_id, user_id, message_count=10
            )
            ```
        """
        logger.info(
